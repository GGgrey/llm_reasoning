...
model_paths = {
    "llama-3.1-8b": "/root/siton-data-0072803f053947c8bb3fe64d115b30e3/models/Llama-3.1-8B-Instruct",
    "Qwen2.5-7B": "/root/siton-data-0072803f053947c8bb3fe64d115b30e3/models/Qwen2.5-7B-Instruct",
    "Ministral-8B-Instruct-2410": "/root/siton-data-0072803f053947c8bb3fe64d115b30e3/models/Ministral-8B-Instruct-2410",
}
...
class TestTimeCompute:
    def __init__(self, task, args):
        ...
        self.gpt("Init")
        if self.args.baseline == "multi_llm_naive" or self.args.baseline == "multi_llm_adaptive_majority" or self.args.baseline == "multi_llm_adaptive_consensus":  # Only for multi-llm methods
            self.gpt_2nd = partial(gpt, model=args.model_2nd, temperature=args.temperature, top_p=args.top_p, max_tokens=args.max_tokens, gpu_memory_utilization = args.inference_gpu_memory_utilization_2nd, model_index="2nd")
            self.gpt_2nd("Init")
            self.gpt_3rd = partial(gpt, model=args.model_3rd, temperature=args.temperature, top_p=args.top_p, max_tokens=args.max_tokens, gpu_memory_utilization = args.inference_gpu_memory_utilization_3rd, model_index="3rd")
            self.gpt_3rd("Init")
        if self.args.baseline == "mutual_consistency":
            # Mutual model config
            model_path = model_paths.get(self.args.mutual_model)
            self.mutual_model = LLM(model=model_path, tensor_parallel_size=1, gpu_memory_utilization=self.args.inference_gpu_memory_utilization_mutual, max_model_len=self.args.max_tokens)
            self.mutual_tokenizer = AutoTokenizer.from_pretrained(model_path)
    ...
    def generate_sentences(self, prompt, n_samples, stop=None, model_index="", baseline=""):
        if model_index == "":
            generated = self.gpt(prompt=prompt, n=n_samples, stop=stop, baseline=baseline)
        elif model_index == "2nd":
            generated = self.gpt_2nd(prompt=prompt, n=n_samples, stop=stop, baseline=baseline)
        elif model_index == "3rd":
            generated = self.gpt_3rd(prompt=prompt, n=n_samples, stop=stop, baseline=baseline)

        if self.args.baseline == "multi_llm_naive" or self.args.baseline == "multi_llm_adaptive_majority" or self.args.baseline == "adaptive_paraphrase_majority" or self.args.baseline == "confidence_weighted_majority":
            return generated
        else:
    ...
    def solve(self, x, idx, to_print=True):
        ...
        elif self.args.baseline == "multi_llm_naive" and self.args.mode == "step_level_solitaire":
            return self.solve_multi_llm_naive_solitaire(x, idx, to_print)
        elif self.args.baseline == "multi_llm_naive" and self.args.mode == "confidence_boost":
            return self.solve_multi_llm_naive_confidence_boost(x, idx, to_print)
        elif self.args.baseline == "multi_llm_adaptive_majority":
            return self.solve_multi_llm_adaptive_majority(x, idx, to_print)
        elif self.args.baseline == "multi_llm_adaptive_consensus":
            return self.solve_multi_llm_adaptive_consensus(x, idx, to_print)
        elif self.args.baseline == "adaptive_paraphrase_majority":
            return self.solve_adaptive_paraphrase_majority(x, idx, to_print)
        elif self.args.baseline == "confidence_weighted_majority":
            return self.solve_confidence_weighted_majority(x, idx, to_print)
        elif self.args.baseline == "mutual_consistency":
            return self.solve_mutual_consistency(x, idx, to_print)
        elif self.args.baseline == "cer":
            return self.solve_cer(x, idx, to_print)
        else:
            raise ValueError(f"Unknown method: {self.args.baseline}.")
    ...
    def solve_multi_llm_naive_solitaire(self, x, idx, to_print=True):
        if self.args.prompt_sample == "cot":
            ...
            raise NameError
        current_prompt += "Answer: Step"

        model_1 = self.gpt(cmd="1st")
        model_2 = self.gpt_2nd(cmd="2nd")
        model_3 = self.gpt_3rd(cmd="3rd")
        model_name = ""

        step_data = []
        step_index = 0
        current_solution = "Answer: Step"
        count = 0
        model_index = ""
        while True:
            count = (count % 3) + 1
            if count == 1:
                model_index = ""
                model_name = model_1.get_model_name()
            elif count == 2:
                model_index = "2nd"
                model_name = model_2.get_model_name()
            elif count == 3:
                model_index = "3rd"
                model_name = model_3.get_model_name()
            else:
                raise ValueError("Invalid number")
            output_infos = self.generate_sentences(current_prompt, self.args.n_generate_sample, stop=self.stop, baseline="multi_llm_naive", model_index=model_index)
            step_info, output_token_ids, cumulative_logprobs, logprobs = output_infos
            step = [g.strip() for g in step_info]
            step_text = step[0].strip()

            # Append to current prompt
            current_prompt += " " + step_text
            current_solution += " " + step_text
            ...
            if "final answer is" in step_text or "End of answer." in step_text or step_index >= 20:
                break
    ...
    def solve_multi_llm_naive_confidence_boost(self, x, idx, to_print=True):
        ...
        while True:
            ...
            # Step confidence check and boost
            retry_count = 0
            max_retry_count = 5
            retry_step_info = []
            retry_prob_info = []
            while retry_count <= max_retry_count:
                output_infos = self.generate_sentences(current_prompt, self.args.n_generate_sample, stop=self.stop, baseline="multi_llm_naive", model_index=model_index)
                step_info, output_token_ids, cumulative_logprobs, logprobs = output_infos
                output_token_length = len(output_token_ids[0])
                print('\n' + '#'*100)
                print("output_token_ids: ", output_token_ids)
                print("output_token_length: ", output_token_length)
                print("cumulative_logprobs: ", cumulative_logprobs)
                print('#'*100 + '\n')
                if output_token_length == 0:
                    continue
                average_prob = math.exp(cumulative_logprobs[0] / output_token_length)
                step = [g.strip() for g in step_info]
                step_text = step[0].strip()
            
                retry_step_info.append(step_text)
                retry_prob_info.append(average_prob)

                if average_prob >= self.args.conf_thresh:
                    break
                else:
                    retry_count += 1
                    count = (count % 3) + 1
            ...
            # Sort based on prob
            if len(retry_step_info) == 1:
                step_text = retry_step_info[0]
            else:
                step_scores = list(zip(retry_step_info, retry_prob_info))
                sorted_step_scores = sorted(step_scores, key=lambda x: x[1], reverse=True)
                step_text = sorted_step_scores[0][0]
    ...
    def solve_multi_llm_adaptive_consensus(self, x, idx, to_print=True):
        ...
        while retry_count < max_retry:
            ...
            count = 0
            candidates_count = 0
            completed_solutions_info = []
            completed_solutions = []
            most_frequent_answer_between_windows = []
            while candidates_count < self.args.n_generate_sample:
                # Get model index and name
                count = (count % 3) + 1
                if count == 1:
                    model_index = ""
                    model_name = model_1.get_model_name()
                ...
                # Generate candidate answers within window
                ...
                # Prepare the info dictionary to store each candidate and its value within the window
                window_info = []
                for candidate in candidates:
                    completed_solutions.append(candidate)
                    completed_solutions_info.append({"prompt": x, "candidate": candidate, "model": model_name})
                    window_info.append({"prompt": x, "candidate": candidate, "model": model_name})

                # Extract and clean answers from the candidates within the window
                extracted_window_answers = [
                    self.task.extract_answer(candidate).replace(": ", "").strip() for candidate in candidates
                ]

                # Count occurrences of each extracted answer within the window
                window_answer_counts = Counter(extracted_window_answers)

                # Ensure no empty answers ("") are selected as the final answer
                sorted_window_answers = window_answer_counts.most_common()
                most_common_window_answer = None

                for answer, _ in sorted_window_answers:
                    if answer != "":  # Skip empty answers
                        most_common_window_answer = answer
                        break

                if most_common_window_answer:
                    # Find the first candidate corresponding to the most common extracted answer within the window
                    most_common_window_answer_count = window_answer_counts[most_common_window_answer]
                    matching_window_candidate = next(
                        candidate for candidate, extracted_window_answer in zip(candidates, extracted_window_answers)
                        if extracted_window_answer == most_common_window_answer
                    )
                    most_frequent_answer_between_windows.append(matching_window_candidate)
                    
                    # Inner-Consistency
                    if most_common_window_answer_count == self.args.window_size:
                        # Optional: Print the matching candidate within the window
                        if to_print:
                            print(matching_window_candidate)
                        return [matching_window_candidate], {"info": window_info}  # If the most common answer count is equal to the window size, return the candidate directly, else continue to the next iteration
                    else:
                        candidates_count += self.args.window_size

            # Extract and clean answers from the complete set of candidates
            extracted_answers = [
                self.task.extract_answer(candidate).replace(": ", "").strip()
                for candidate in most_frequent_answer_between_windows
            ]

            # Count occurrences of each extracted answer from the complete set of candidates
            ...
            if most_common_answer:
                # Find the first candidate corresponding to the most common extracted answer from the complete set of candidates
                most_common_answer_count = answer_counts[most_common_answer]
                matching_candidate = next(
                    candidate for candidate, extracted_answer in zip(most_frequent_answer_between_windows, extracted_answers)
                    if extracted_answer == most_common_answer
                )

                # Outer-Consistency
                if most_common_answer_count > len(most_frequent_answer_between_windows):
                    # Optional: Print the matching candidate
                    if to_print:
                        print(matching_candidate)
                    return [matching_candidate], {"info": completed_solutions_info}
                else:  # Mutual-Consistency
                    pass  # Todo
    ...
    def solve_multi_llm_adaptive_majority(self, x, idx, to_print=True):
        ...
        while retry_count < max_retry:
            ...
            while candidates_count < self.args.n_generate_sample:
                ...
                # Generate candidate answers within window
                responses, output_token_ids, cumulative_logprobs, logprobs = self.generate_sentences(
                    current_prompt, 
                    self.args.window_size, 
                    stop="End of answer.",
                    model_index=model_index,
                ...
                candidates = [r.strip() for r in responses]
                # Prepare the info dictionary to store each candidate and its value within the window
                window_info = []
                for candidate, logp, tokens in zip(candidates, cumulative_logprobs, output_token_ids):
                    average_logp = math.exp(logp / len(tokens))
                    completed_solutions.append(candidate)
                    completed_solutions_logp.append(average_logp)
                    completed_solutions_info.append({"prompt": x, "candidate": candidate, "model": model_name, "logp": average_logp})
                    window_info.append({"prompt": x, "candidate": candidate, "model": model_name, "logp": average_logp, "window": True})
                ...
                if most_common_window_answer:
                    ...
                    # Check if the most common answer count is equal to the window size
                    if most_common_window_answer_count == self.args.window_size:
                        return [matching_window_candidate], {"info": window_info}  # If the most common answer count is equal to the window size, return the candidate directly, else continue to the next iteration
                    else:
                        candidates_count += self.args.window_size

            # Get average logp and prune solutions
            index_to_remove = []
            average_logp = sum(completed_solutions_logp) / len(completed_solutions_logp)
            print("Average logp: ", average_logp)
            for i in range(0, len(completed_solutions_logp)):
                if completed_solutions_logp[i] < average_logp:
                    index_to_remove.append(i)
            completed_solutions = [item for idx, item in enumerate(completed_solutions) if idx not in index_to_remove]

            # Extract and clean answers from the complete set of candidates
            extracted_answers = [
                self.task.extract_answer(candidate).replace(": ", "").strip()
                for candidate in completed_solutions
            ]
            ...
            if most_common_answer:
                # Find the first candidate corresponding to the most common extracted answer from the complete set of candidates
                matching_candidate = next(
                    candidate for candidate, extracted_answer in zip(completed_solutions, extracted_answers)
                    if extracted_answer == most_common_answer
                )
    ...
    def solve_adaptive_paraphrase_majority(self, x, idx, to_print=True, max_retry=5):
        ...
        while retry_count < max_retry:
            ...
            completed_solutions = []
            completed_solutions_logp = []
            rephrased_question_temp = ""
            output_question = ""
            prompt_type = "paraphrase_question"
            window_answer_counts = []
            while candidates_count < self.args.n_generate_sample:
                # Rephrase question
                if candidates_count > 0:
                    model = self.gpt(cmd="1st")
                    sampling_params = SamplingParams(
                        ...
                        n=1,
                        include_stop_str_in_output=True,
                        stop="End of paraphrase.",
                        ignore_eos=False,
                    )
                    if prompt_type == "paraphrase_question":
                        rephrased_question_temp = self.task.paraphrase_question_prompt_wrap(x, "")
                        rephrased_question = model.model.generate(
                            rephrased_question_temp, 
                            sampling_params
                        )
                        output_question = rephrased_question[0].outputs[0].text
                        end_pos = output_question.find("End of paraphrase.")
                        if end_pos != -1:
                            output_question = output_question[:end_pos]
                            output_question.strip()
                        start_pos = output_question.find("Rephrased Question:")
                        if start_pos != -1:
                            index = len("Rephrased Question:")
                            output_question = output_question[index:]
                            output_question.strip()
                
                        if self.args.prompt_sample == "standard":
                            current_prompt = self.task.standard_prompt_wrap(output_question, "")
                        elif self.args.prompt_sample == "cot":
                            current_prompt = self.task.cot_prompt_wrap(output_question, "")
                        elif self.args.prompt_sample == 'reflect_cot':
                            current_prompt = self.task.reflect_cot_prompt_wrap(output_question, "")
                        else:
                            raise ValueError("Invalid prompt_sample mode")          
                ...
                for i in range(0, len(candidates)):
                    candidate_logp = []
                    for logprob, token_id in zip(logprobs[i], output_token_ids[i]):
                        candidate_logp.append(math.exp(logprob[token_id].logprob))
                    logprob_infos.append(candidate_logp)

                window_info = []
                average_logp = -1
                average_compute_logp = -1
                min_compute_logp = -1
                dot_compute_logp = -1
                for candidate, logp, tokens, logp_info in zip(candidates, cumulative_logprobs, output_token_ids, logprob_infos):
                    if len(tokens) != 0:
                        average_logp = math.exp(logp / len(tokens))
                        average_compute_logp = sum(logp_info) / len(logp_info)
                        min_compute_logp = min(logp_info)
                        dot_compute_logp = reduce(operator.mul, logp_info, 1)
                    completed_solutions.append(candidate)
                    completed_solutions_logp.append(average_logp)
                    completed_solutions_info.append({"prompt": x, "rephrased_question": output_question, "candidate": candidate, "average_logp": average_logp, "average_compute_logp": average_compute_logp, "min_compute_logp": min_compute_logp, "dot_compute_logp": dot_compute_logp})
                    window_info.append({"prompt": x, "rephrased_question": output_question, "candidate": candidate, "average_logp": average_logp, "average_compute_logp": average_compute_logp, "min_compute_logp": min_compute_logp, "dot_compute_logp": dot_compute_logp, "window": True})
            ...
            # Get high-confidence solutions and low-confidence solutions with their confidence scores         
            completed_solutions_high = [item for idx, item in enumerate(completed_solutions) if idx not in index_to_remove]
            completed_solutions_low = [item for idx, item in enumerate(completed_solutions) if idx in index_to_remove]
            completed_solutions_logp_high = [item for idx, item in enumerate(completed_solutions_logp) if idx not in index_to_remove]
            completed_solutions_logp_low = [item for idx, item in enumerate(completed_solutions_logp) if idx in index_to_remove]
            """
    ...
    def solve_confidence_weighted_majority(self, x, idx, to_print):
        ...
        responses, output_token_ids, cumulative_logprobs, logprobs = self.generate_sentences(current_prompt, N, stop="End of answer.", baseline="confidence_weighted_majority")
        candidates = [r.strip() for r in responses]
        ...
        # Caculate confidence scores
        candidates_probs = []
        for candidate_token, candidate_logprob in zip(output_token_ids, logprobs):
            candidate_prob = []
            for i in range(0, len(candidate_token)):
                if len(candidate_logprob[i]) > 0:
                    candidate_prob.append(math.exp(list(candidate_logprob[i].values())[0].logprob))
            candidates_probs.append(candidate_prob)

        min_weighted_scores = calculate_confidence_score(candidates_probs, "min", True)
        average_weighted_scores = calculate_confidence_score(candidates_probs, "perplexity", True)
        average_low_weighted_scores = calculate_confidence_score(candidates_probs, "average_low", True)
        dot_weighted_scores = calculate_confidence_score(candidates_probs, "dot", True)
        perplexity_weighted_scores = calculate_confidence_score(candidates_probs, "perplexity", True)
        ...
        for answer, _ in sorted_answers:
            if answer != "":
                ...
                break
        if most_common_answer:
            majority_matching_candidate = next(
                candidate for candidate, extracted_answer in zip(candidates, extracted_answers)
                if extracted_answer == most_common_answer
            )
        matching_candidates.append(majority_matching_candidate)

        # Confidence Weighted Majority Vote
        min_weighted_votes = {}
        average_weighted_votes = {}
        average_low_weighted_votes = {}
        dot_weighted_votes = {}
        perplexity_weighted_votes = {}
        for idx, answer in enumerate(extracted_answers):
            min_weight = min_weighted_scores[idx]  # Get the normalized score of the candidate
            if answer in min_weighted_votes:
                min_weighted_votes[answer] += min_weight  # Add score to the total for this answer
            else:
                min_weighted_votes[answer] = min_weight  # Initialize score for this answer

            average_weight = average_weighted_scores[idx]
            if answer in average_weighted_votes:
                average_weighted_votes[answer] += average_weight
            else:
                average_weighted_votes[answer] = average_weight

            average_low_weight = average_low_weighted_scores[idx]
            if answer in average_low_weighted_votes:
                average_low_weighted_votes[answer] += average_low_weight
            else:
                average_low_weighted_votes[answer] = average_low_weight
            
            dot_weight = dot_weighted_scores[idx]
            if answer in dot_weighted_votes:
                dot_weighted_votes[answer] += dot_weight
            else:
                dot_weighted_votes[answer] = dot_weight

            perplexity_weight = perplexity_weighted_scores[idx]
            if answer in perplexity_weighted_votes:
                perplexity_weighted_votes[answer] += perplexity_weight
            else:
                perplexity_weighted_votes[answer] = perplexity_weight

        # Select the answer with the highest total weighted vote
        min_most_voted_answer = max(min_weighted_votes, key=min_weighted_votes.get)
        average_most_voted_answer = max(average_weighted_votes, key=average_weighted_votes.get)
        average_low_most_voted_answer = max(average_low_weighted_votes, key=average_low_weighted_votes.get)
        dot_most_voted_answer = max(dot_weighted_votes, key=dot_weighted_votes.get)
        perplexity_most_voted_answer = max(perplexity_weighted_votes, key=perplexity_weighted_votes.get)

        # Filter candidates that match the most voted answer
        min_matching_candidates = [
            candidates[i] for i, ans in enumerate(extracted_answers) if ans == min_most_voted_answer
        ]

        average_matching_candidates = [
            candidates[i] for i, ans in enumerate(extracted_answers) if ans == average_most_voted_answer
        ]

        average_low_matching_candidates = [
            candidates[i] for i, ans in enumerate(extracted_answers) if ans == average_low_most_voted_answer
        ]

        dot_matching_candidates = [
            candidates[i] for i, ans in enumerate(extracted_answers) if ans == dot_most_voted_answer
        ]

        perplexity_matching_candidates = [
            candidates[i] for i, ans in enumerate(extracted_answers) if ans == perplexity_most_voted_answer
        ]

        # Evaluate these matching candidates and select the one with the best value
        min_matching_values = [min_weighted_scores[i] for i, ans in enumerate(extracted_answers) if ans == min_most_voted_answer]
        min_best_idx = np.argmax(np.array(min_matching_values))

        average_matching_values = [average_weighted_scores[i] for i, ans in enumerate(extracted_answers) if ans == average_most_voted_answer]
        average_best_idx = np.argmax(np.array(average_matching_values))

        average_low_matching_values = [average_low_weighted_scores[i] for i, ans in enumerate(extracted_answers) if ans == average_low_most_voted_answer]
        average_low_best_idx = np.argmax(np.array(average_low_matching_values))

        dot_matching_values = [dot_weighted_scores[i] for i, ans in enumerate(extracted_answers) if ans == dot_most_voted_answer]
        dot_best_idx = np.argmax(np.array(dot_matching_values))

        perplexity_matching_values = [perplexity_weighted_scores[i] for i, ans in enumerate(extracted_answers) if ans == perplexity_most_voted_answer]
        perplexity_best_idx = np.argmax(np.array(perplexity_matching_values))

        min_best_candidate = min_matching_candidates[min_best_idx]
        min_best_value = min_matching_values[min_best_idx]

        average_best_candidate = average_matching_candidates[average_best_idx]
        average_best_value = average_matching_values[average_best_idx]

        average_low_best_candidate = average_low_matching_candidates[average_low_best_idx]
        average_low_best_value = average_low_matching_values[average_low_best_idx]

        dot_best_candidate = dot_matching_candidates[dot_best_idx]
        dot_best_value = dot_matching_values[dot_best_idx]

        perplexity_best_candidate = perplexity_matching_candidates[perplexity_best_idx]
        perplexity_best_value = perplexity_matching_values[perplexity_best_idx]

        # Prepare the info dictionary to store each candidate, its value, and extracted answer
        info = []
        for candidate, extracted_answer in zip(candidates, extracted_answers):
            info.append({"x": x, "candidate": candidate, "extracted_answer": extracted_answer})

        info.append({"majority vote": majority_matching_candidate, "min": min_best_candidate, "min_best_value": min_best_value, "average": average_best_candidate, "average_best_value": average_best_value, "average_low": average_low_best_candidate, "average_low_best_value": average_low_best_value, "dot": dot_best_candidate, "dot_best_value": dot_best_value, "perplexity": perplexity_best_candidate, "perplexity_best_value": perplexity_best_value})

        matching_candidates.append(min_best_candidate)
        matching_candidates.append(average_best_candidate)
        matching_candidates.append(average_low_best_candidate)
        matching_candidates.append(dot_best_candidate)
        matching_candidates.append(perplexity_best_candidate)
    ...
    def solve_mutual_consistency(self, x, idx, to_print):
        ...
        extracted_answers = []
        for candidate in candidates:
            extracted_answers.append(self.task.extract_answer(candidate).replace(": ", "").strip())
            info.append({"prompt": x, "candidate": candidate})

        # Count occurrences of each extracted answer, if all the answers are the same, return the answer directly
        answer_counts = Counter(extracted_answers)
        if len(answer_counts) == 1:
            matching_candidate = candidates[0]
            return [matching_candidate], {"info": info}

        # Adaptive mutual reasoning
        ...
        for candidate in candidates:
            ...
            prefixs = []
            candidate_index += 1
            index = candidate.find("so the final answer is:")
            if index != -1:
                candidate_copy = candidate[:index].strip()
            else:
                candidate_copy = candidate
            mutual_index = get_random_points(len(candidate_copy), self.args.mutual_iteration)
            for index in mutual_index:
                prefix = candidate[:index].strip()
                prefixs.append(prefix)
                input_prompt = current_prompt + " " + prefix
                mutual_prompts.append(input_prompt)

            # Generate mutual reasoning solutions
            mutual_candidates = self.mutual_model.generate(mutual_prompts, self.sampling_params)

            # Get mutual reasoning answers
            responses = []
            for candidate in mutual_candidates:
                for item in candidate.outputs:
                    responses.append(item.text)
            
            # Connect prefix and mutual solution
            extracted_mutual_answers = []
            for prefix, response in zip(prefixs, responses):
                # Calculate completion_tokens
                self.mutual_completion_tokens += len(self.mutual_tokenizer.encode(response))
                solution = prefix + response
                mutual_answer = self.task.extract_answer(solution).replace(': ', '').strip()
                extracted_mutual_answers.append(mutual_answer)
                extracted_answers.append(mutual_answer)
                answer_counts.update([mutual_answer])
                all_candidates.append(solution)
                info.append({"prompt": x, "prefix": prefix, "response": response})

            # Check for mutual consistency
            mutual_answer_counts = Counter(extracted_mutual_answers)
            if len(mutual_answer_counts) == 1:
                if extracted_mutual_answers[0] == extracted_answers[candidate_index]:
                    return [all_candidates[-1]], {"info": info}

        # Majority voting for all the solutions
        # Ensure no empty answers ("") are selected as the final answer
        sorted_answers = answer_counts.most_common()
        most_common_answer = None
        ...
        if most_common_answer:
            # Find the first candidate corresponding to the most common extracted answer
            matching_candidate = next(
                candidate for candidate, extracted_answer in zip(all_candidates, extracted_answers)
                if extracted_answer == most_common_answer
            )
    ...
    def self_refine(self, input, solution, average_logp, iteration):
        ...
        for i in range(iteration):
            feedback_ans = self.generate_sentences(feedback_prompt.format(question=input, solution=current_ans), 1)[0].strip()
            if 'No error' in feedback_ans:
                break
            responses, output_token_ids, cumulative_logprobs, logprobs = self.generate_sentences(refine_prompt.format(question=input, solution=current_ans, feedback=feedback_ans), 1, baseline=self.args.baseline)
            current_ans = responses[0].strip()
            if len(output_token_ids[0]) != 0:
