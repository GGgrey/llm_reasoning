...
model_2nd_instance = None
model_3rd_instance = None
model_mutual_instance = None


def initialize_model(model_name="llama-2-7b", gpu_memory_utilization=0.9, model_index=""):
    global open_model_instance, reward_model_instance, model_2nd_instance, model_3rd_instance, model_mutual_instance
    model_instance = None
    if model_name.startswith("llama-") or model_name.startswith("Qwen2") or model_name.startswith("Mistral") or model_name.startswith("QwQ") or model_name.startswith("Ministral"):
        
        if open_model_instance is None and model_index == "":
        ...
        if model_2nd_instance is None and model_index == "2nd":
            model_2nd_instance = Model(model_name=model_name, gpu_memory_utilization= gpu_memory_utilization)
            model_instance = model_2nd_instance
        if model_3rd_instance is None and model_index == "3rd":
            model_3rd_instance = Model(model_name=model_name, gpu_memory_utilization= gpu_memory_utilization)
            model_instance = model_3rd_instance
        if model_mutual_instance is None and model_index == "mutual":
            model_mutual_instance = Model(model_name=model_name, gpu_memory_utilization= gpu_memory_utilization)
            model_instance = model_mutual_instance
...
def gpt(cmd="", prompt="", model="gpt-4o", temperature=0.7, max_tokens=2000, n=1, stop=None, top_p=0.9, gpu_memory_utilization=0.9, model_index="", baseline=""):
    # Initialize the model if it's not already initialized
    if cmd == "Init":
        initialize_model(model_name=model, gpu_memory_utilization=gpu_memory_utilization, model_index=model_index)
        return
    elif cmd == "1st":
        return open_model_instance
    elif cmd == "2nd":
        return model_2nd_instance
    elif cmd == "3rd":
        return model_3rd_instance
    elif cmd == "mutual":
        return model_mutual_instance
    
    global completion_tokens, prompt_tokens
    messages = [{"role": "user", "content": prompt}]
    outputs = []
    if model == "gpt-4o":  # For GPT-4o API call
        return gpt4o_ask(messages, temperature, max_tokens, n, stop,top_p)
    elif model.startswith("llama-") or model.startswith("Qwen2") or model.startswith("Mistral") or model.startswith("QwQ") or model.startswith("Ministral"):
        initialize_model(model_name=model, gpu_memory_utilization=gpu_memory_utilization, model_index=model_index)
        ...
        elif model_index == "2nd":  # Only for multi-llm methods
            outputs = model_2nd_instance.predict(
                prompt = prompt,
                temperature=temperature,
                max_tokens=max_tokens,
                n=n,
        ...
        elif model_index == "3rd":  # Only for multi-llm methods
            outputs = model_3rd_instance.predict(
                prompt = prompt,
                temperature=temperature,
                max_tokens=max_tokens,
                n=n,
        ...
        elif model_index == "mutual":
            outputs = model_mutual_instance.predict(
                prompt = prompt,
                temperature=temperature,
                max_tokens=max_tokens,
                n=n,
        ...
        if baseline == "multi_llm_naive" or baseline == "multi_llm_adaptive_majority" or baseline == "adaptive_paraphrase_majority" or baseline == "confidence_weighted_majority":
            output_token_ids = [output["output_token_ids"] for output in outputs]
            cumulative_logprobs = [output["cumulative_logprob"] for output in outputs]
            logprobs = [output["logprobs"] for output in outputs]
            return responses, output_token_ids, cumulative_logprobs, logprobs
        else:
            return responses
    elif model.startswith("internlm"):  # For reward models
